...
/*   Updated: 2022/09/28 15:10:09 by xxxxxxx          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int tourner_dans_le_vide_vide(t_fdf *fdf)
{
	...
	static double angle = M_PI / 3.42;


	if (fdf->dx < 50 || fdf->dx > 1910 + 250)
		{
			angle = -angle - M_PI;
			r = rand() % 155 + 100;
		g = rand() % 155 + 100;
	...
	if (fdf->dy < -40 || fdf->dy > 1090 + 20)
		...
		fdf->bg_color = r << 16 | g << 8 | b;
			...
			fdf->dx += cos(angle) * SPEED;
			fdf->dy += sin(angle) * SPEED;

	ft_bzero(fdf->px_addr, fdf->line_bytes * fdf->h);
	draw_tab(fdf);
	mlx_put_image_to_window(fdf->img_ptr, fdf->win_ptr, fdf->img_ptr, 0, 0);
	mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, 150, 50, mlx_get_color_value(fdf->mlx_ptr, 0x00FFFFFF), "17h23 a 42 Mulhouse");
	mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, 180, 80, mlx_get_color_value(fdf->mlx_ptr, 0x00FFFFFF), "29-09-2022");
	mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, 35, 110, mlx_get_color_value(fdf->mlx_ptr, 0x00FFFFFF), "By iCARUS & xxxxxxx - students since 21/02/2022");
	usleep(16000);
	return (0);
}
...
static void	init_fdf(t_fdf *fdf)
{
	...
	fdf->win_ptr = mlx_new_window(fdf->mlx_ptr, fdf->w, fdf->h, "17h23 42 Mulhouse");
